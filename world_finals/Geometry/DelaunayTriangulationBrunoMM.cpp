namespace delaunay {
	...
	pair<Q, Q> build_tr(vector<point>& p, int l, int r) {
		...
		while (true) {
			if (left(lb->o, ra->o, ra->dest())) ra = ra->rev()->prev();
			else if (left(lb->o, ra->o, lb->dest())) lb = lb->rev()->next();
			else break;
		}
		Q b = conn(lb->rev(), ra);
		auto valid = [&](Q e) { return left(e->dest(), b->dest(), b->o); };
	...
	vector<vector<int>> delaunay(vector<point> v) {
		...
		bool col = true;
		for (int i = 2; i < n; i++) if (area2(v[i], v[i-1], v[i-2])) col = false;
		if (col) {
			for (int i = 1; i < n; i++)
